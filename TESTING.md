...
- Headers: `.md` and `/llms.txt` include `Content-Type`, `X-Content-Type-Options`, `ETag`, and `Last-Modified`; `.md` includes `Link: <canonical HTML URL>; rel="canonical"` and sends `X-Robots-Tag: noindex` when `md_send_noindex` is enabled; `/llms.txt` sends `X-Robots-Tag: noindex` when `llms_send_noindex` is enabled; neither header includes `nofollow`; `.md` returns `304` only for matching ETags so metadata-only changes are not hidden by `If-Modified-Since`.
