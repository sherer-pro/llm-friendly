...
These tests use WordPress stubs to cover Markdown helper safety, settings sanitization, `llms.txt` section order, Markdown canonical/noindex headers, export eligibility, and Gutenberg block conversion for links, files, embeds, details, nested content, and code fences.
...
- Essential links: save valid and invalid `llms_essential_links` lines (`Title | URL | Notes`), confirm unsafe protocols are dropped, site-relative URLs are expanded in `llms.txt`, and the section appears after "Main links" and before recent post-type sections.
...
- Headers: `.md` and `/llms.txt` include `Content-Type`, `X-Content-Type-Options`, `ETag`, and `Last-Modified`; `.md` includes `Link: <canonical HTML URL>; rel="canonical"` and sends `X-Robots-Tag: noindex` when enabled; `/llms.txt` sends `X-Robots-Tag: noindex` without `nofollow`; `.md` returns `304` only for matching ETags so metadata-only changes are not hidden by `If-Modified-Since`.
- AI crawler diagnostics: verify the settings page shows OAI-SearchBot, GPTBot, Googlebot/Search AI features, Google-Extended, and the configured sitemap URL, and confirm the plugin does not modify `robots.txt` automatically.
