...
You can opt in to descriptions in llms.txt via `llms_show_excerpt`, and Markdown exports send `X-Robots-Tag: noindex` by default via `md_send_noindex` so Markdown-only endpoints do not compete with canonical HTML pages.
...
* llms.txt endpoint with cached generation, curated Essential links, optional AI-facing descriptions, a configurable custom Markdown notes block, and a per-post exclusion list.
...
* Optional X-Robots-Tag: noindex for both /llms.txt and Markdown exports; the Markdown header is controlled by `md_send_noindex`.
...
Enable the “Send X-Robots-Tag: noindex for Markdown” option (stored as `md_send_noindex`) to emit the header on all Markdown responses.
